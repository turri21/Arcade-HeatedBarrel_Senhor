...
#(parameter W=5,
            ...
            MFREQ=48000  // clk input frequency in kHz (48MHz)
)(
    input         rst,
    input         clk,
    output   reg  cpu_cen,
...
// MISTER_SIM: fast-cen mode — alternate cen/cenb every cycle so the CPU
// runs at full clk/2 instead of the divided 8/10 MHz. Speeds up Verilator
// sim significantly without breaking HW (gated by `ifdef`).
`ifdef MISTER_SIM
wire fast_cen_mode = 1'b1;
`else
wire fast_cen_mode = 1'b0;
`endif
always @(posedge clk) begin
    cencnt  <= cencnt_nx[CW] ? {CW{1'b1}} : cencnt_nx[CW-1:0];
    if( rst ) cencnt <= 0;
    if( fast_cen_mode || over || rst || recover) begin
        cpu_cen  <=  risefall;
        cpu_cenb <= ~risefall;
        risefall <= ~risefall;
    end else begin
